...
def start_rover():
    ...
    while True:
        ...
        if run_status == 1:
            ...
            upload_file_to_dropbox(local_file_path, dropbox_destination_path, access_token)
            upload_file_to_dropbox(local_file_path_nav, dropbox_destination_path_nav, access_token)
            upload_file_to_dropbox(local_file_path_pos, dropbox_destination_path_pos, access_token)
            if(base_point==1):
            ...
            ref_status.update(data_status)
            download_file_from_dropbox(local_file_path_base,dropbox_base_path,access_token)
            ...
            ref_status.update(data_status)
            upload_file_to_dropbox(local_file_path_pos_post, dropbox_destination_path_post, access_token)
            upload_data_post()
            ref = db.reference(f"/Realtime/base/")
            data = {
                f"request": 0
