...
import dropbox  # Pastikan Anda sudah menginstal dropbox dengan `pip install dropbox`
...
# Firebase configuration (gunakan file kredensial yang sesuai)
cred_ta = credentials.Certificate("/home/ronny/asset/ppls-app.json")
shift_app = firebase_admin.initialize_app(
    cred_ta, {"databaseURL": "https://ppls-app-default-rtdb.firebaseio.com/"}
)

# Konfigurasi parameter lainnya
device_port = "/dev/ttyGPS"
baud_rate = "115200"
output_raw_file = "/home/ronny/gnss-rover/output.ubx"
output_rinex_file = "/home/ronny/gnss-rover/output.obs"
output_rinex_file_nav = "/home/ronny/gnss-rover/output.nav"
rtkbin_path = "/path/to/rtklib/bin"
tcp_port = 9000

# Fungsi untuk meng-upload file ke Dropbox
def upload_file_to_dropbox(file_path, dropbox_path, access_token):
    # Menghubungkan ke Dropbox dengan access token
    dbx = dropbox.Dropbox(access_token)
    try:
        with open(file_path, "rb") as file:
            # Upload file ke Dropbox, menggunakan mode overwrite untuk menggantikan file yang ada
            dbx.files_upload(file.read(), dropbox_path, mode=dropbox.files.WriteMode("overwrite"))
        print(f"File '{file_path}' uploaded successfully to {dropbox_path}")
    except Exception as e:
        print(f"Error uploading file: {e}")

# Main script untuk upload .obs dan .nav ke Dropbox
def main():
    # Tentukan path file lokal
    local_file_path_obs = "/home/ronny/gnss-rover/output.obs"
    local_file_path_nav = "/home/ronny/gnss-rover/output.nav"

    # Tentukan path tujuan di Dropbox
    dropbox_path_obs = "/GPS ZED-F9P/Rover/output.obs"
    dropbox_path_nav = "/GPS ZED-F9P/Rover/output.nav"
...
# Fungsi untuk mengumpulkan data GNSS menggunakan str2str
def collect_raw_data(duration):
    ...
    str2str_command = [
        f"str2str",
        "-in",
        f"serial:///ttyGPS:115200:8:n:1",
        "-out",
        f"tcpsvr://:{tcp_port}",
        "-out",
        f"file://{output_raw_file}",
        "-msg",
        "1003,1004,1011,1012,1019,1020,1045,1044,1046,1074,1084,1094,1124,1077,1087,1097,1127",
    ]
...
# Fungsi untuk mengkonversi raw data ke format RINEX menggunakan RTKLIB
        ...
        with open(file_path, "r") as file:
            try:
                for line_number, line in enumerate(file, start=0):
                    ...
                    delta = np.array(
                        [x_rover - x_ref, y_rover - y_ref, z_rover - z_ref]
                    )
                    ...
                    # print(enu)
                    upload_data["east"].append({f"{waktu}": enu[0]})
                    upload_data["north"].append({f"{waktu}": enu[1]})
                    upload_data["up"].append({f"{waktu}": enu[2]})
            finally:
...
# Fungsi utama untuk menjalankan rover GNSS
def start_rover():
    run_status = 0
    local_file_path = "/home/ronny/gnss-rover/output.obs"
    local_file_path_nav = (
        "/home/ronny/gnss-rover/output.nav"  # Path to your local .txt file
    )
    local_file_path_pos = (
        "/home/ronny/gnss-rover/solution1.pos"  # Path to your local .txt file
    )
    local_file_path_pos_post = (
        "/home/ronny/gnss-rover/solution_post.pos"  # Path to your local .txt file
    )
    local_file_path_base = (
        "/home/ronny/gnss-rover/base.obs"  # Path to your local .txt file
    )
    ...
    while True:
        ...
        print(run_status)
        if run_status == 1:
            data_status = {f"status": "Setting Up"}
            ref_status.update(data_status)
            run_status = 0
            ref = db.reference("/Realtime")
            data = ref.child("rover").get()
            ...
            upload_file_to_dropbox(
                local_file_path, dropbox_destination_path, access_token
            )
            upload_file_to_dropbox(
                local_file_path_nav, dropbox_destination_path_nav, access_token
            )
            upload_file_to_dropbox(
                local_file_path_pos, dropbox_destination_path_pos, access_token
            )
            if base_point == 1:
                ref = db.reference("Realtime", shift_app)
                ...
                data = ref.child("base").get()
                dropbox_base_path = data["obs"]
            data_status = {f"status": "Processing Result..."}
            ref_status.update(data_status)
            download_file_from_dropbox(
                local_file_path_base, dropbox_base_path, access_token
            )
            # ref = db.reference(f"/Patok/{index}/Storage")
            # data = {
            ...
            data = {f"{label}": dropbox_destination_path}
            ref.update(data)
            ref = db.reference(f"/Patok/{index}/Storage/nav")
            data = {f"{label}": dropbox_destination_path_nav}
            ref.update(data)
            ref = db.reference(f"/Patok/{index}/Storage/pos")
            data = {f"{label}": dropbox_destination_path_pos}
            ref.update(data)
            ref = db.reference(f"/Patok/{index}/Storage/post")
            data = {f"{label}": dropbox_destination_path_post}
            ref.update(data)
            ref = db.reference(f"/Patok/{index}/Realtime")
            data = {"time": f"{tanggal}"}
            ref.update(data)
            run_rnx2rtkp()
            data_status = {f"status": "Uploading POST Result..."}
            ref_status.update(data_status)
            upload_file_to_dropbox(
                local_file_path_pos_post,
                dropbox_destination_path_post,
                access_token,
            )
            upload_data_post()
            ref = db.reference(f"/Realtime/base/")
    ...
    while True:
        ...
        if run_status_rcv == 1:
            ...
            try:
                ...
                print(
                    f"RTKRCV timed out after {duration} seconds. Proceeding with the program..."
                )
            except subprocess.CalledProcessError as e:
...
def run_rnx2rtkp():
    ...
    rtkrcv_command = [
        ...
        "-k",
        "/home/ronny/gnss-rover/post.conf",
        "-o",
        "/home/ronny/gnss-rover/solution_post.pos",  # Path to rtkrcv configuration
    ]
    ...
    try:
        ...
        print(
            f"RNX2RTKP timed out after {duration} seconds. Proceeding with the program..."
        )
    ...
    while True:
        ...
        if run_status_real == 1:
            ...
            if "GPST" in last_line or "%" in last_line or len(last_line) <= 2:
                ...
                data = {"lat": x, "long": y, "alt": z, "qua": q, "ns": ns}
                ref = db.reference("/Realtime/rover")
                ref.update(data)
                if q < 3:
                    if startup_flag == 0:
                        ref = db.reference(f"Patok/{index}/Realtime")
                        data = {"lat": x, "long": y, "alt": z}
                        ref.update(data)
                        startup_flag = 1
                    ref = db.reference(f"Patok/{index}")
                    data = ref.child("Realtime").get()
                    ...
                    enu = np.dot(rot_matrix, delta)
                    ref = db.reference(f"Patok/{index}/Realtime")
                    data = {"east": enu[0], "north": enu[1], "up": enu[2]}
                    ref.update(data)
                ref = db.reference(f"Patok/{index}/Realtime")
                data = {"status": 1}
                ref.update(data)
