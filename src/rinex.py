...
output_rinex_file_nav = (
    "/home/ronny/gnss-rover/output.nav"  # Final RINEX observation file
)
...
rtkrcv_command = [
    "rtkrcv",
    "-o",
    "/home/ronny/gnss-rover/ntrip1.conf",  # Path to rtkrcv configuration
    "-s",  # Start immediately
]
...
def start_rover():
    ...
    while True:
        ...
        if run_status == 1:
            ...
            upload_file_to_dropbox(
                local_file_path_pos, dropbox_destination_path_pos, new_access_token
            )
            if base_point == 1:
            ...
            download_file_from_dropbox(
                local_file_path_base, dropbox_base_path, new_access_token
            )
            # ref = db.reference(f"/Patok/{index}/Storage")
            # data = {
